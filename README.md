...
- Explanation Type Diversity(ETD): Quantifies the diversity of explanation types for the recommended products provided to a user.
  Given a user $u$ with a top-k list of recommended products \( \tilde{P}_u \) and the corresponding explanation paths \( \hat{L}_u \), let \( \𝜔_{\hat{L}_u} = \{ \𝜔_l \mid l \in \hat{L}_u \} \) be the set of unique path types in the explanations for the recommended list. ETD is calculated as:
